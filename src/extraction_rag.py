...
def get_lang_query(string: str) -> str:
     if(Lang == Lang.EN):
         return "search_queries_EN"
     if(Lang == Lang.ZH_SIM):
         return "search_queries_ZH_SIM"
     if(Lang == Lang.ZH_TR):
         return "search_queries_ZH_TR"
...
def extract_s3_1(report, model: str = "gpt-4o-mini"):
    """
    ...
    # print(f"DEBUG - Financial context for S3.1:\n{financial_context}")
    ...
    if TARGET_LANGUAGE == Lang.EN:
        prompt = f"""
            ...
            - Do NOT calculate new metrics or invent data
            ...
            - Revenue Growth trends from 2022→2023→2024
...
def extract_s3_2(report, model: str = "gpt-4o-mini"):
    """
    Extract Section 3.2 Financial Performance Summary based on Section 2 financial data.
    Analyzes 5 key perspectives: Comprehensive financial health, Profitability and earnings quality,
    Operational efficiency, Financial risk identification and early warning, Future financial performance projection.
    """
    global TARGET_LANGUAGE
    ...
    # Prepare comprehensive financial data context for LLM
    financial_context = f"""
    ...
    Operating Expense: 2024: {inc.operating_expense.year_2024}, 2023: {inc.operating_expense.year_2023}, 2022: {inc.operating_expense.year_2022}
    Net Profit: 2024: {inc.net_profit.year_2024}, 2023: {inc.net_profit.year_2023}, 2022: {inc.net_profit.year_2022}
    Income Before Taxes: 2024: {inc.income_before_income_taxes.year_2024}, 2023: {inc.income_before_income_taxes.year_2023}, 2022: {inc.income_before_income_taxes.year_2022}
    Income Tax Expense: 2024: {inc.income_tax_expense.year_2024}, 2023: {inc.income_tax_expense.year_2023}, 2022: {inc.income_tax_expense.year_2022}
    Interest Expense: 2024: {inc.interest_expense.year_2024}, 2023: {inc.interest_expense.year_2023}, 2022: {inc.interest_expense.year_2022}
    Currency: {getattr(inc, 'primary_currency', 'N/A')}, Multiplier: {getattr(inc, 'primary_multiplier', 'N/A')}
    
    BALANCE SHEET DATA:
    Total Assets: 2024: {bal.total_assets.year_2024}, 2023: {bal.total_assets.year_2023}, 2022: {bal.total_assets.year_2022}
    Current Assets: 2024: {bal.current_assets.year_2024}, 2023: {bal.current_assets.year_2023}, 2022: {bal.current_assets.year_2022}
    Non-Current Assets: 2024: {bal.non_current_assets.year_2024}, 2023: {bal.non_current_assets.year_2023}, 2022: {bal.non_current_assets.year_2022}
    Total Liabilities: 2024: {bal.total_liabilities.year_2024}, 2023: {bal.total_liabilities.year_2023}, 2022: {bal.total_liabilities.year_2022}
    Current Liabilities: 2024: {bal.current_liabilities.year_2024}, 2023: {bal.current_liabilities.year_2023}, 2022: {bal.current_liabilities.year_2022}
    Non-Current Liabilities: 2024: {bal.non_current_liabilities.year_2024}, 2023: {bal.non_current_liabilities.year_2023}, 2022: {bal.non_current_liabilities.year_2022}
    Shareholders Equity: 2024: {bal.shareholders_equity.year_2024}, 2023: {bal.shareholders_equity.year_2023}, 2022: {bal.shareholders_equity.year_2022}
    Retained Earnings: 2024: {bal.retained_earnings.year_2024}, 2023: {bal.retained_earnings.year_2023}, 2022: {bal.retained_earnings.year_2022}
    Inventories: 2024: {bal.inventories.year_2024}, 2023: {bal.inventories.year_2023}, 2022: {bal.inventories.year_2022}
    
    CASH FLOW DATA:
    Net Cash from Operations: 2024: {cf.net_cash_from_operations.year_2024}, 2023: {cf.net_cash_from_operations.year_2023}, 2022: {cf.net_cash_from_operations.year_2022}
    Net Cash from Investing: 2024: {cf.net_cash_from_investing.year_2024}, 2023: {cf.net_cash_from_investing.year_2023}, 2022: {cf.net_cash_from_investing.year_2022}
    Net Cash from Financing: 2024: {cf.net_cash_from_financing.year_2024}, 2023: {cf.net_cash_from_financing.year_2023}, 2022: {cf.net_cash_from_financing.year_2022}
    Net Increase/Decrease in Cash: 2024: {cf.net_increase_decrease_cash.year_2024}, 2023: {cf.net_increase_decrease_cash.year_2023}, 2022: {cf.net_increase_decrease_cash.year_2022}
    Dividends: 2024: {cf.dividends.year_2024}, 2023: {cf.dividends.year_2023}, 2022: {cf.dividends.year_2022}
    
    KEY FINANCIAL METRICS:
    Gross Margin: 2024: {metrics.gross_margin.year_2024}, 2023: {metrics.gross_margin.year_2023}, 2022: {metrics.gross_margin.year_2022}
    Operating Margin: 2024: {metrics.operating_margin.year_2024}, 2023: {metrics.operating_margin.year_2023}, 2022: {metrics.operating_margin.year_2022}
    Net Profit Margin: 2024: {metrics.net_profit_margin.year_2024}, 2023: {metrics.net_profit_margin.year_2023}, 2022: {metrics.net_profit_margin.year_2022}
    Current Ratio: 2024: {metrics.current_ratio.year_2024}, 2023: {metrics.current_ratio.year_2023}, 2022: {metrics.current_ratio.year_2022}
    Debt to Equity: 2024: {metrics.debt_to_equity.year_2024}, 2023: {metrics.debt_to_equity.year_2023}, 2022: {metrics.debt_to_equity.year_2022}
    Return on Equity: 2024: {metrics.return_on_equity.year_2024}, 2023: {metrics.return_on_equity.year_2023}, 2022: {metrics.return_on_equity.year_2022}
    Return on Assets: 2024: {metrics.return_on_assets.year_2024}, 2023: {metrics.return_on_assets.year_2023}, 2022: {metrics.return_on_assets.year_2022}
    Effective Tax Rate: 2024: {metrics.effective_tax_rate.year_2024}, 2023: {metrics.effective_tax_rate.year_2023}, 2022: {metrics.effective_tax_rate.year_2022}
    Interest Coverage: 2024: {metrics.interest_coverage.year_2024}, 2023: {metrics.interest_coverage.year_2023}, 2022: {metrics.interest_coverage.year_2022}
    Asset Turnover: 2024: {metrics.asset_turnover.year_2024}, 2023: {metrics.asset_turnover.year_2023}, 2022: {metrics.asset_turnover.year_2022}
    ...
    if TARGET_LANGUAGE == Lang.EN:
        prompt = f"""
        You are a senior financial analyst preparing a comprehensive Financial Performance Summary based on the company's financial data from 2022-2024.
        
        INSTRUCTIONS:
        - Analyze ONLY the provided financial data from Section 2
        - Do NOT calculate new metrics or invent data
        - Use exact numbers and percentages as provided
        - Focus on multi-year trends, patterns, and comprehensive business insights
        - Provide detailed analysis for both 2024 and 2023 reports
        - Structure your analysis around the five key perspectives below
        
        Analyze the following five perspectives for BOTH 2024 and 2023:
        ...
        - Margin analysis (gross, operating, net profit margins)
        - Return ratios (ROE, ROA) and earnings sustainability
        
        3. Operational Efficiency
        - Operating margin performance and cost management
        - Asset utilization and turnover ratios
        - Cash flow from operations and working capital management
        
        4. Financial Risk Identification and Early Warning
        - Leverage ratios and debt management
        - Liquidity ratios and solvency indicators
        - Tax rate changes and other risk factors
        
        5. Future Financial Performance Projection
        - Investment activities and growth indicators
        - Cash flow sustainability and dividend policy
        - Market expansion and business development trends
        
        Return your analysis as JSON with this exact structure (10 keys fixed):
        {{
            "comprehensive_financial_health_2024": "Detailed analysis of 2024 financial position using specific data points",
            "comprehensive_financial_health_2023": "Detailed analysis of 2023 financial position using specific data points",
            "profitability_earnings_quality_2024": "Analysis of 2024 profitability and earnings quality with specific metrics",
            "profitability_earnings_quality_2023": "Analysis of 2023 profitability and earnings quality with specific metrics",
            "operational_efficiency_2024": "Analysis of 2024 operational efficiency using specific operational metrics",
            "operational_efficiency_2023": "Analysis of 2023 operational efficiency using specific operational metrics",
            "financial_risk_identification_2024": "Analysis of 2024 financial risks and early warning indicators",
            "financial_risk_identification_2023": "Analysis of 2023 financial risks and early warning indicators",
            "future_financial_performance_projection_2024": "Projection based on 2024 data and trends",
            "future_financial_performance_projection_2023": "Projection based on 2023 data and trends"
        }}
        ...
        prompt = f"""
            你是一名资深财务分析师，必须用**简体中文**撰写“财务表现总结”。你只能使用【第二部分：财务数据】中提供的数字与百分比，不得编造或推断未提供的数据。

            ## 产出硬性要求（务必遵守）
            - 语言：**仅限简体中文**。JSON 的**键名必须严格使用下方给定的英文键**；所有**字符串值必须是简体中文**。
            - 数据来源：**只**能使用“FINANCIAL DATA”中提供的内容；不得使用外部资料、常识或网络信息。
            - 计算限制：一般情况下**不得**计算新指标或改写数值；不得推断未提供的数据。
            - 缺失处理：若某项信息在提供数据中**不存在或无法直接得出**，对应的字符串值应写**"N/A"**（全大写）。
            - 输出格式：**只输出一个 JSON 对象**，不添加任何解释、前后缀或额外字段；**键名必须与下方结构完全一致**，**不得增删**。

            ## 数字格式化规则（最高优先级，用于单位换算与显示）
            - **严格禁止**在输出中出现**“亿/亿元/亿人民币”等字样**。
            - **统一优先使用“千万”单位**展示大数金额；若不足千万再用“万元”，不足一万保留“元”。
            - 若原始数据单位为**万元**：
            - 若数值 **≥ 1,000**（即 ≥ 一千万）→ **改用“千万元”**，显示数值 = 原数值 ÷ 1,000。
            - 若数值 **< 1,000** → 继续使用“万元”。
            - 若原始数据单位为**元**：
            - 若数值 **≥ 10,000,000 元** → **改用“千万元”**，显示数值 = 元 ÷ 10,000,000。
            - 若 10,000 元 ≤ 数值 < 10,000,000 元 → 使用“万元”，显示数值 = 元 ÷ 10,000。
            - 若数值 < 10,000 元 → 保留“元”。
            - 舍入与显示（仅限上述单位换算）：允许**四舍五入到 1 位小数**；去除无意义尾随“0”和小数点（如“2.0 千万元”→“2 千万元”）。
            - 幣種/原单位说明：不得改变币种含义或报告口径；如原文为“GBP Millions/百万元”等，可在句中注明原单位，但**金额展示仍按本规则以‘千万元/万元/元’表述**（不改变数值含义）。

            ## 分析范围（2024 与 2023 各写一段）
            1. 综合财务健康状况：资产规模变化、负债与权益结构、流动性与资本结构趋势。
            2. 盈利能力和盈利质量：收入与利润趋势、（若已提供）利润率与回报率的变化及稳定性。
            3. 营运效率：营业效率（如已提供的营业利润率）、资产利用与周转、经营性现金流与营运资金管理。
            4. 财务风险识别与预警：杠杆与流动性（仅限已提供指标）、税率变化及其他可从数据直接观察到的风险信号。
            5. 未来财务表现展望：基于**已提供数据的**投资活动、现金流与分红、业务扩张趋势所做的**合规性**展望（不得引入外部信息或新计算）。

            ## 严格的 JSON 输出结构（键名固定为英文；值为简体中文）
            {{
                "comprehensive_financial_health_2024": "用简体中文、基于已给数据写2024年的综合财务健康分析；按数字格式化规则展示；若无可写则填N/A",
                "comprehensive_financial_health_2023": "同上，但针对2023年",
                "profitability_earnings_quality_2024": "用简体中文、基于已给数据写2024年的盈利能力与盈利质量；仅引用已提供指标；按数字格式化规则展示；若无则N/A",
                "profitability_earnings_quality_2023": "同上，但针对2023年",
                "operational_efficiency_2024": "用简体中文、基于已给数据写2024年的营运效率；按数字格式化规则展示；若无则N/A",
                "operational_efficiency_2023": "同上，但针对2023年",
                "financial_risk_identification_2024": "用简体中文、基于已给数据写2024年的风险识别与预警；若无则N/A",
                "financial_risk_identification_2023": "同上，但针对2023年",
                "future_financial_performance_projection_2024": "用简体中文、仅基于已给数据对2024年进行合规性展望；若无则N/A",
                "future_financial_performance_projection_2023": "同上，但针对2023年"
            }}

            ## 语言与合规哨兵
            - 若**任何字符串值**包含“亿/亿元/亿人民币/億/億元”等字样，或包含英文、拼音、URL、emoji，或出现无法由给定数据直接得出的数字/百分比，必须**重新生成**，直到完全符合上述要求。
        ...
        prompt = f"""
            你是一位資深財務分析師，必須用**繁體中文**撰寫「財務表現總結」。你只能使用【第二部分：財務資料】中提供的數字與百分比，不得編造或推斷未提供的資料。

            ## 產出硬性要求（務必遵守）
            - 語言：**僅限繁體中文**。JSON 的**鍵名必須嚴格使用下方給定的英文鍵**；所有**字串值必須是繁體中文**。
            - 資料來源：**只**能使用「FINANCIAL DATA」中的內容；不得使用外部資訊、常識或網路資料。
            - 計算限制：一般情況下**不得**計算新指標或改寫數值；不得推斷未提供的資料。
            - 缺失處理：若某項資訊在提供資料中**不存在或無法直接得出**，對應的字串值應寫**"N/A"**（全大寫）。
            - 輸出格式：**只輸出一個 JSON 物件**，不添加任何解釋、前後綴或額外欄位；**鍵名必須與下方結構完全一致**，**不得增刪**。

            ## 數字格式化規則（最高優先級，用於單位換算與顯示）
            - **嚴禁**在輸出中出現**「億/億元/億新台幣/億人民幣」**等字樣。
            - **統一優先使用「千萬」單位**呈現大數金額；若不足千萬再用「萬元」，不足一萬保留「元」。
            - 若原始資料單位為**萬元**：
            - 若數值 **≥ 1,000**（即 ≥ 一千萬）→ **改用「千萬元」**，顯示數值 = 原數值 ÷ 1,000。
            - 若數值 **< 1,000** → 繼續使用「萬元」。
            - 若原始資料單位為**元**：
            - 若數值 **≥ 10,000,000 元** → **改用「千萬元」**，顯示數值 = 元 ÷ 10,000,000。
            - 若 10,000 元 ≤ 數值 < 10,000,000 元 → 使用「萬元」，顯示數值 = 元 ÷ 10,000。
            - 若數值 < 10,000 元 → 保留「元」。
            - 捨入與顯示（僅限上述單位換算）：允許**四捨五入至 1 位小數**；去除無意義尾隨 0 與小數點（如「2.0 千萬元」→「2 千萬元」）。
            - 幣別/原單位說明：不得改變幣別涵義或報告口徑；如原文為「GBP Millions/百萬元」等，可於語句中註明原單位，但**金額展示仍按本規則以『千萬元/萬元/元』表述**（不改變數值涵義）。

            ## 分析範圍（2024 與 2023 各寫一段）
            1. 綜合財務健康狀況：資產規模變化、負債與權益結構、流動性與資本結構趨勢。
            2. 獲利能力與盈餘品質：營收與獲利趨勢、（若已提供）利潤率與報酬率的變化及穩定性。
            3. 營運效率：營業效率（如已提供的營業利潤率）、資產利用與周轉、營運現金流與營運資金管理。
            4. 財務風險識別與預警：槓桿與流動性（僅限已提供指標）、稅率變化及其他可由資料直接觀察的風險訊號。
            5. 未來財務表現展望：基於**已提供資料**的投資活動、現金流與股利、業務擴張趨勢所做的**合規性**展望（不得引入外部資訊或新計算）。

            ## 嚴格的 JSON 輸出結構（鍵名固定為英文；值為繁體中文）
            {{
                "comprehensive_financial_health_2024": "以繁體中文、基於已給資料撰寫2024年的綜合財務健康分析；依數字格式化規則呈現；若無可寫則填N/A",
                "comprehensive_financial_health_2023": "同上，但針對2023年",
                "profitability_earnings_quality_2024": "以繁體中文、基於已給資料撰寫2024年的獲利能力與盈餘品質；僅引用已提供指標；依數字格式化規則呈現；若無則N/A",
                "profitability_earnings_quality_2023": "同上，但針對2023年",
                "operational_efficiency_2024": "以繁體中文、基於已給資料撰寫2024年的營運效率；依數字格式化規則呈現；若無則N/A",
                "operational_efficiency_2023": "同上，但針對2023年",
                "financial_risk_identification_2024": "以繁體中文、基於已給資料撰寫2024年的風險識別與預警；若無則N/A",
                "financial_risk_identification_2023": "同上，但針對2023年",
                "future_financial_performance_projection_2024": "以繁體中文、僅基於已給資料對2024年進行合規性展望；若無則N/A",
                "future_financial_performance_projection_2023": "同上，但針對2023年"
            }}

            ## 語言與合規哨兵
            - 若**任何字串值**包含「億/億元/億人民幣/億新台幣」等字樣，或包含英文、拼音、URL、emoji，或出現無法由給定資料直接得出的數字/百分比，必須**重新生成**，直到完全符合上述要求。
    ...
    try:
        ...
        return {
            "comprehensive_financial_health_2024": result.get("comprehensive_financial_health_2024", "N/A"),
            "comprehensive_financial_health_2023": result.get("comprehensive_financial_health_2023", "N/A"),
            "profitability_earnings_quality_2024": result.get("profitability_earnings_quality_2024", "N/A"),
            "profitability_earnings_quality_2023": result.get("profitability_earnings_quality_2023", "N/A"),
            "operational_efficiency_2024": result.get("operational_efficiency_2024", "N/A"),
            "operational_efficiency_2023": result.get("operational_efficiency_2023", "N/A"),
            "financial_risk_identification_2024": result.get("financial_risk_identification_2024", "N/A"),
            "financial_risk_identification_2023": result.get("financial_risk_identification_2023", "N/A"),
            "future_financial_performance_projection_2024": result.get("future_financial_performance_projection_2024", "N/A"),
            "future_financial_performance_projection_2023": result.get("future_financial_performance_projection_2023", "N/A")
        }
...
def extract_s3_3(md_file_2024: str, md_file_2023: str, top_k: int = 15, model: str = "gpt-4o-mini"):
    """
    ...
    # Separate RAG searches for Business Model
    print("🔍 Searching for Business Model information...")
...
def _extract_business_competitiveness_single_year(context: str, year: str, model: str):
    """Extract business competitiveness for a single year with combined business model and market position context"""
    global TARGET_LANGUAGE
    ...
    if TARGET_LANGUAGE == Lang.EN:
        ...
        INSTRUCTIONS:
        - Use ONLY the provided text from the {year} annual report contexts
        ...
        COMBINED CONTEXT FROM {year} ANNUAL REPORT:
        {context}
        """
    ...
    try:
        response = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": "You are an expert business analyst. Extract business model and market position information from annual reports. Return valid JSON only."},
                {"role": "user", "content": prompt}
            ],
            temperature=0,
            max_tokens=1500
        ...
        print(f"Error in _extract_business_competitiveness_single_year for {year}: {e}")
        
        
def extract_s4_1(md_file_2024: str, md_file_2023: str, top_k: int = 15, model: str = "gpt-4o-mini"):
    """
...
def _extract_risk_factors_single_year(context: str, year: str, model: str):
    ...
    if TARGET_LANGUAGE == Lang.EN:
        ...
        - 1–3 concise sentences per risk category
...
def extract(md_file1: str, md_file2: str, *, currency_code: str = "USD", target_lang: Lang = Lang.EN):
    ...
    profitability_analysis = extract_s3_1(report, model="gpt-4o-mini")
    ...
    financial_performance_summary = extract_s3_2(report, model="gpt-4o-mini")
    ...
    risk_factors = extract_s4_1(md_file_2024, md_file_2023, top_k=15, model="gpt-4o-mini")
    ...
    challenges_uncertainties = extract_s6_2(md_file_2024, md_file_2023, top_k=15, model="gpt-4o-mini")
    ...
    innovation_development = extract_s6_3(md_file_2024, md_file_2023, top_k=15, model="gpt-4o-mini")
